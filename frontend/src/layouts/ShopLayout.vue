...
const displayedProducts = computed(() => {
    ...
    if (!Array.isArray(props.books)) {
        console.warn('books is not an array:', props.books);
        return [];
    }
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                <div v-for="book in displayedProducts" :key="book.id" class="bg-white p-4 rounded-lg shadow-md">
                    <img :src="book.image" :alt="book.name" class="w-full h-48 object-cover mb-4 rounded" />
                    <h3 class="text-lg font-semibold mb-2 line-clamp-1">{{ book.name }}</h3>
                    <p class="text-gray-400 text-xs line-clamp-1">{{ book.description }}</p>
